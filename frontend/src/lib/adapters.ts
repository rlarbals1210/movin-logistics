...
export const 예시_추천콜: 운송인추천콜[] = [
  { 콜ID: 'CALL-1042', 톤급: 5, 출발지: '경기 화성시 향남읍', 도착지: '충북 청주시 흥덕구', 거리km: 118.4, 예측_운임: 352000, 톨비: 6800, 표준소요_h: 2.1 },
  { 콜ID: 'CALL-1087', 톤급: 5, 출발지: '인천 서구 가좌동', 도착지: '경북 구미시 산동읍', 거리km: 287.6, 예측_운임: 615000, 톨비: 18400, 표준소요_h: 4.3 },
  { 콜ID: 'CALL-1113', 톤급: 5, 출발지: '부산 강서구 미음동', 도착지: '전남 광양시 황금동', 거리km: 164.2, 예측_운임: 428000, 톨비: 9700, 표준소요_h: 2.6 },
]
...
function 추천콜로(row: Record<string, unknown>): 운송인추천콜 | undefined {
  ...
  const 톤 = 숫자로(값찾기(row, 추천콜_별칭.톤급))
  if (콜ID === undefined || 톤 === undefined || !톤급_목록.includes(톤 as 톤급)) return undefined
 ...
 * 축이 죽었는지 본다.
 *
 * 10:58 산출물은 5·11·25톤 결과가 원 단위까지 같아서 18칸 그리드가 6칸으로 보였다.
